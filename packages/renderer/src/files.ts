...
/**
 * Read-only file access. The renderer reads its inputs — theme files, `static/`,
 * pack files — through this seam, so the same code runs against a real
 * filesystem (the CLI) or an in-memory tree (the Builder — TDD §6.6, §15.2).
 *
 ...
 * Writing stays in `write.ts`, which is Node-only by design.
 */
