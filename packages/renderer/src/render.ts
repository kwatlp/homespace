...
export interface RenderInput {
  ...
  /** Homespace root directory (source of static/, theme/, and pack files). */
  root: string;
...
/** Resolve a homespace-relative path, or null if it escapes the homespace root. */
...
/**
 * Render a homespace to a set of static files + an asset copy plan. Pure with
...
export async function render(input: RenderInput): Promise<RenderResult> {
  ...
  const tokens = renderTokensCss(homespace.theme?.tokens);
  ...
  if (typeof homespace.theme?.css === "string") {
    ...
    else warnings.push({ severity: "warning", message: `theme.css '${homespace.theme.css}' not found in the homespace` });
  }

  // 2. Pre-read html-section files (operator-authored, confined to the homespace).
  const sections = Array.isArray(homespace.sections) ? homespace.sections : [];
  const htmlFiles = new Map<string, string>();
  for (const section of sections) {
    if (section.type !== "html" || typeof section.file !== "string") continue;
    const abs = confinedPath(root, section.file);
    if (abs === null) {
      errors.push({ severity: "error", message: `html section file '${section.file}' escapes the homespace root` });
      continue;
    }
    if (!(await exists(abs))) {
  ...
  const title = homespace.title ?? homespace.name;
  const layout = homespace.layout === "pages" || homespace.layout === "grid" ? homespace.layout : "scroll";
  const sitemapPaths = ["/"];

  const useThumbs = input.thumbnails === true;
  const renderInto = (section: Section, basePrefix: string): string =>
    renderSection(section, { homespace, catalog, basePrefix, htmlFiles, thumbnails: useThumbs }) ?? "";

  if (layout === "scroll") {
    const main = supported.map((s) => renderInto(s, "")).join("\n");
    files.push({ path: "index.html", contents: page({ homespace, title, basePrefix: "", main, feed: hasFeed }) });
  } else {
    ...
    if (layout === "pages") {
      ...
      files.push({ path: "index.html", contents: page({ homespace, title, basePrefix: "", main, nav: navFor(""), feed: hasFeed }) });
      ...
      files.push({ path: "index.html", contents: page({ homespace, title, basePrefix: "", main: gridMain, nav: navFor(""), feed: hasFeed }) });
      for (const m of metas) emitSectionPage(m.section, m.slug);
    }
  }

  // 4. Detail pages + pack file copies (generated regardless of layout).
  const detailCtx: RenderContext = { homespace, catalog, basePrefix: DETAIL_PREFIX };
  ...
  if (feedPosts.length > 0) {
    files.push({ path: "feed.xml", contents: renderFeed(homespace, feedPosts, site) });
  }
